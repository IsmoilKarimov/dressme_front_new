...
function FootwearCheckFilter({ footWear, onFootWearGetValue }) {
                    ...
                    <figure className="w-full flex flex-wrap justify-start gap-x-[2px] gap-y-2">
                        {footwearData?.slice(0, visibleButtons)?.map((footwear, index) => {
                            return (
                                <button
                                    key={index + 1}
                                    onClick={() => onHandleUnderWearList(footwear)}
                                    className={`${dataActionUnderwearSizes === footwear
