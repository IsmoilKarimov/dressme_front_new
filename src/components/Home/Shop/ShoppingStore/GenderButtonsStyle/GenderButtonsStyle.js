...
function GenderButtonsStyle({ handleGetId }) {
  ...
  return (
    ...
    // <section className="w-fit md:w-[480px] flex items-center border border-searchBgColor rounded-xl bg-slate-50 md:mt-0">
    //   {genderCategory?.map((data) => {
    //     return (
    //       <div
    //         key={data?.id}
    //         className="w-1/4 flex justify-between h-11 rounded-xl"
    //       >
    //         <button
    //           key={data?.id}
    //           onClick={() => {
    ...
    //           <span>{data.icon}</span>
    //           {data.name ? (
    //             <p className="ml-2 text-borderWinter">{data.name}</p>
    //           ) : (
    //             ""
    //           )}
    //         </button>
