...
function CategoryFootwearSizesFilter({
  state,
  setState,
  filter,
            ...
            {footwearData?.slice(0, visibleButtons)?.map((footwear, index) => {
              return (
                <button
                  ...
                  } h-10 w-[57px] flex items-center justify-center not-italic font-AeonikProMedium leading-3 text-center text-black bg-bgCategory  hover:bg-fullBlue  hover:text-white transition ease-linear duration-200 rounded-lg`}
                >
                  <div className="flex items-center">
                    <span>{footwear?.size}</span>
                    <span className={`${dataActionFootwearSizes === index ? 'block' : 'hidden'} ml-1`}>({filterData?.category_products?.total || 0})</span>
                  </div>
                </button>
              );
            })}
...
export default React.memo(CategoryFootwearSizesFilter);
