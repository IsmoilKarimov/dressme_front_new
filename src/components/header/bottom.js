  ...
  return (
    <nav className="w-full flex flex-col justify-center items-center m-0 p-0 box-border ss:hidden md:block">
      ...
      <section className="max-w-[1280px] w-[100%] flex justify-between items-center m-auto ">
        ...
        <button
          className="relative w-[180px] gap-x-1 px-2 h-[44px] rounded-xl bg-btnBgColor overflow-hidden border-searchBgColor border  flex items-center justify-between  cursor-pointer select-none group ml-2"
          // onClick={openPrizeModal}
          onClick={() => {
            setPriceModalOpen(true);
          }}
        >
          {dressInfo?.mainRangePrice?.length > 2 && <div className={` absolute font-AeonikProRegular categoryLinearPrice left-0 w-full h-full z-[10] top-0`}></div>}
