...
function MarketFilterofMaps({ onClick }) {
      ...
      <div>
        <div className="w-full h-[300px] overflow-auto VerticelScroll flex flex-col  bg-white">
          {clothesList?.map((item) => {
            return (
              <div
                onClick={() => handleClothesList(item?.id)}
                key={item?.id}
                className="w-full h-[44px] flex items-center justify-between cursor-pointer pt-5 pb-[8px] border-b border-searchBgColor"
              >
  ...
  const [componentGroup, setComponentGroup] = useState([
    ...
    {
      id: 2,
      pageName: <ClothesList />,
      noActiveIcons: <ClothesIcons colors={"#000"} />,
      ActiveIcons: <ClothesIcons colors={dressInfo?.ColorSeason} />,
      isActive: false,
      text: "По категории",
    },
    {
      id: 3,
      pageName: <PriceRange />,
      noActiveIcons: <DollorIcons colors={"#000"} />,
      ActiveIcons: <DollorIcons colors={dressInfo?.ColorSeason} />,
      isActive: false,
      text: "По бюджету",
    },
    {
      id: 4,
      pageName: <GenderSelect />,
      noActiveIcons: <ManWomGenBlack colors={"#000"} />,
      ActiveIcons: <ManWomGenBlack colors={dressInfo?.ColorSeason} />,
      isActive: false,
      text: "По полу",
    },
    {
      id: 5,
      pageName: <BrandMarketSelect />,
      noActiveIcons: <TopBrandsIcon colors={"#000"} />,
      ActiveIcons: <TopBrandsIcon colors={dressInfo?.ColorSeason} />,
      isActive: false,
      text: "По бренду",
    },
  ]);
  ...
  return (
    <div className="w-full h-full  py-5 px-4">
      ...
      {componentGroup?.map((data) => {
        return <div className="mt-4">{data?.isActive && data?.pageName}</div>;
      })}
    </div>
  );
}
